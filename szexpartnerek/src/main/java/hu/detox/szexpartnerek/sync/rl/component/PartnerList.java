package hu.detox.szexpartnerek.sync.rl.component;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.node.ArrayNode;
import hu.detox.szexpartnerek.sync.IPersister;
import hu.detox.szexpartnerek.sync.ITrafoEngine;
import hu.detox.szexpartnerek.sync.rl.component.sub.Partner;
import hu.detox.szexpartnerek.sync.rl.persister.PartnerListPersister;
import org.jsoup.Jsoup;
import org.jsoup.internal.StringUtil;
import org.jsoup.nodes.Element;
import org.springframework.stereotype.Component;

import java.util.*;
import java.util.function.Function;

@Component
public class PartnerList implements ITrafoEngine {
    private final ITrafoEngine[] pre;
    private final Set<Integer> ids;
    private final transient PartnerListPersister persister;

    private PartnerList(Partner p) {
        pre = new ITrafoEngine[]{p};
        persister = new PartnerListPersister();
        ids = new HashSet<>();
    }

    @Override
    public void reStart(boolean force) {
        if (force) persister.init();
    }

    @Override
    public Function<String, String> url() {
        return rest -> {
            if (StringUtil.isBlank(rest)) {
                rest = "40";
            }
            if (StringUtil.isNumeric(rest)) {
                rest = "rosszlanyok.php?pid=szexpartner-lista&htext=" + rest;
            }
            return rest;
        };
    }

    @Override
    public Iterator<?> input(JsonNode parent) {
        if (parent instanceof ArrayNode an) return an.iterator();
        return null;
    }

    @Override
    public ITrafoEngine[] preTrafos() {
        return pre;
    }

    @Override
    public IPersister persister() {
        return persister;
    }

    @Override
    public Iterator<String> pager() {
        ids.clear();
        return new Iterator<>() {
            int page = 0;

            @Override
            public boolean hasNext() {
                return true;
            }

            @Override
            public String next() {
                return "page=" + page++;
            }
        };
    }

    @Override
    public Object apply(String resp) {
        var soup = Jsoup.parse(resp);
        var list = soup.select("div.listOuterDiv");
        List res = new LinkedList();
        var titles = soup.select("h1#listTitleH1,h2.subTitle,title");
        String title = null;
        for (Element el : titles) {
            String text = el.text();
            if (title == null || (text.length() < title.length() && text.length() > 1)) {
                title = text;
            }
        }
        boolean idChanged = false;
        for (Element mainDiv : list) {
            var hn = mainDiv.selectFirst("div.hiddenNev");
            if (hn == null) continue;
            Integer id = Integer.parseInt(mainDiv.attr("data-memberthumb"));
            String hiddenNev = hn.text();
            if (!ids.add(id)) continue;
            idChanged = true;
            String age = mainDiv.selectFirst("div.nev").text().replaceAll(".+\\(([0-9+]+)\\)", "$1");
            var lti = mainDiv.select("img.listTagImage");
            String pic = lti.attr("data-src");
            if (pic.isEmpty()) {
                pic = lti.attr("src");
            }
            res.add(new Object[]{id, hiddenNev, age, pic});
        }
        if (!idChanged) {
            ids.clear();
        }
        return idChanged ? Map.of("title", title, "list", res) : null;
    }
}
