...
if "x"=="x%LAUNCHER%" set LAUNCHER=submodule\launcher
if x%UTILS%==x set UTILS=detox-*.jar
...
if not "x%DEBUG:covc=%"=="x%DEBUG%" set "DTX_DJARGS=%DTX_DJARGS% -javaagent:ext/jacocoagent.jar=destfile=%TARGET%\%DTX_EXECUTABLE%.exec,append=true"
if not "x%DEBUG:covf=%"=="x%DEBUG%" set "DTX_DJARGS=%DTX_DJARGS% -javaagent:ext/jacocoagent.jar=destfile=%TARGET%\%DTX_EXECUTABLE%.exec,append=false,jmx=true"
...
set DTX_CLASSPATH=%DTX_PROFILE_DIR%/ext/cp;%JDIR%/cp;%DTX_CLASSPATH%;%LAUNCHER%
...
if x%LAUNCH%==x (
	set LAUNCH=false
	if exist %LAUNCHER% set LAUNCH=true
    if %JAVAVER% gtr 17 set LAUNCH=false
)
...
if exist "%UPDATE%" (
	if %BATCHUPDATE%==1 (
		...
		set NUX_SHELL=%DTX_SHELL%
		if NOT %NUX_SHELL%x==x set NUX_SHELL=%NUX_SHELL% /WAIT
		%NUX_SHELL% "%DTX_JAVA_EXECUTABLE%" %DTX_DJARGS% -cp "%DTX_CLASSPATH%" -Dtarget=%TARGET% -Dbase=%BASE% hu.detox.launcher.Main
	)
)
if %LAUNCH% == true (
	%DTX_SHELL% "%DTX_JAVA_EXECUTABLE%" %DTX_JARGS% -DstdIn=%STDIN% "-Dagents=%UTILS%=a:" -cp "%DTX_CLASSPATH%" -Dtarget=%TARGET% -Dbase=%BASE% -Dmain_class=%DTX_MAIN_CLASS% hu.detox.launcher.Main %*
) else (
	%DTX_SHELL% "%DTX_JAVA_EXECUTABLE%" %DTX_JARGS% -javaagent:%UTILS% -cp "%DTX_CLASSPATH%;%UTILS%" "-javaagent:%ASPECTJ%" -DstdIn=%STDIN% -Dtarget=%TARGET% -Dbase=%BASE% %DTX_MAIN_CLASS% %*
)